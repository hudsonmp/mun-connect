    ...
    examples = [
        ...
        },
        {
            ...
            "content": "Position Paper: The Delegation of France on the Topic of Climate Change "
                     ...
                     "reduce our carbon emissions by 40% by 2030. "
...
def train_model(train_dataset, val_dataset, vocab_size, embedding_dim=100, hidden_dim=128, 
    ...
    for epoch in range(epochs):
        ...
        print(f"Epoch {epoch+1}/{epochs}, Loss: {total_loss/len(train_loader):.4f}, Validation Accuracy: {val_accuracy:.4f}")
    
    print("Training completed")
    return model

def predict_document_type(text, model, vocab, device):
    """Predict document type for a given text"""
    # Convert text to dataset format
    dataset = TextClassificationDataset([text], [0], vocab)  # Label doesn't matter here
    
    # Get model prediction
    model.eval()
    with torch.no_grad():
        inputs = dataset[0]["text"].unsqueeze(0).to(device)  # Add batch dimension
        outputs = model(inputs)
        _, predicted = torch.max(outputs, 1)
        
    return IDX_TO_DOC_TYPE[predicted.item()]
...
def main():
    ...
    print("Document classifier training starting...")
    ...
    if args.use_example_data:
        ...
        documents = create_example_training_data()
    else:
        print(f"Loading processed data from {args.processed_dir}...")
        documents = load_processed_data(args.processed_dir)
    
    if not documents:
        print("No documents found or created. Exiting.")
        return
    ...
    labels = [DOC_TYPE_TO_IDX[doc["document_type"]] for doc in documents]
    ...
    val_dataset = TextClassificationDataset(val_texts, val_labels, vocab=train_dataset.vocab)
    ...
    test_model(test_dataset, model, device, output_file)
    
    # Save model and vocabulary
    print("Saving model and vocabulary...")
    model_path = os.path.join(args.output_dir, "document_classifier.pt")
